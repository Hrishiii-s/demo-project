...
const Card = ({ id, onClick, children, style }) => (
  <div className="flex justify-center items-center">
    <div
      className={`radial rounded-full p-6 w-24 h-24 flex items-center justify-center text-center shadow-lg ${style}`}
      onClick={() => onClick(id)}
    >
      {children}
    </div>
          ...
          >
            ...
            >
              <div className="flip-container">
                <img
                  src="/assets/img/icon/chip.webp"
                  className="w-20 h-20 flip-image"
                  alt="chip"
                />
              </div>
            </Card>
            <h5
              className={`${i === 2 ? 'top-36' : i === 6 ? 'bottom-32 left-5' : i > 2 && i < 6 ? 'right-44 top-8 w-56 text-right' : 'left-40 top-9 w-72 h-10'} absolute`}
            >
              {' '}
              {icon ? icon.name : 'Unknown'}{' '}
            </h5>
          </div>
        </div>
      </>
